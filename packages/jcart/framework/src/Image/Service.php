...
namespace Jcart\Framework\Image;
...
class Service
{
     ...
     * @return \Jcart\Framework\Image\LocalFile
     */
    public function upload($image, $path = null, $makeDiffSizes = true)
    {
        $dbPath = $image->store($path, 'jcart');  
        $name = basename($dbPath);

        if (true === $makeDiffSizes) {
            $sizes = config('jcart-framework.image.sizes');
     ...
     *
     * @var string
     * @return \Jcart\Framework\Image\Service
     */
