...
class Product extends BaseModel
{
     ...
     *
     * @param array $$data
     * @return \App\Models\Product $this
     */
    ...
    protected function saveProductDownloadable($data)
    {
        if (isset($data['downloadable']) && count($data['downloadable']) > 0) {
            ...
            $dbPath = $mainDownloadableMedia->store($path, 'public');
            $token = str_random(32);
            ...
            if (null !== $demoDownloadableMedia) {
                $tmpPath = str_split(strtolower(str_random(3)));
                $path = 'uploads/downloadables/' . implode('/', $tmpPath);
                $demoDbPath = $demoDownloadableMedia->store($path, 'public');
     ...
     *
     * @param \App\Models\AttributeDropdownOption
